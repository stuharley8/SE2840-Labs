...
class BusTracker {
	constructor() {		
        ...
        const handleSuccess = (response, textStatus, jqXHR) => {
            ...
            if(response.status !== undefined) {     // Handles route error number's 1000 and 1002
                $("#errorLabel").empty().append(response.status).show();
                return;
            } else if(response["bustime-response"].error !== undefined) {   // Handles route error number 1003
                $("#errorLabel").empty().append(response["bustime-response"].error[0].msg).show();
                return;
            }
            update++;
            $("#update").append(`Update: ${update}`);
            $("#thead").empty().append(`<tr><th>Bus</th><th>Route ${route}</th><th>latitude</th><th>longitude</th><th>speed(MPH)</th><th>dist(mi)</th></tr>`);
            ...
            response["bustime-response"].vehicle.forEach((vehicle) => {
                let tablebodyhtml = `<tr><td>${vehicle.vid}</td><td>${vehicle.des}</td><td>${vehicle.lat}</td><td>${vehicle.lon}</td><td>${vehicle.spd}</td>`;
                let dist = vehicle.pdist / FT_IN_MILE;
                tablebodyhtml += `<td>${dist}</td></tr>`;
                $("#tbody").append(tablebodyhtml);
